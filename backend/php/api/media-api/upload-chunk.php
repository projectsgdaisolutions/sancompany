...
$totalSize = (int) ($_POST['total_size'] ?? $_GET['total_size'] ?? 0);
$originalFilename = trim((string) ($_POST['original_filename'] ?? $_GET['original_filename'] ?? ''));
$mimeType = strtolower(trim((string) ($_POST['mime_type'] ?? $_GET['mime_type'] ?? '')));
...
if ($chunkIndex >= $totalChunks) {
    errorResponse('Chunk index is outside the upload.', 400);
}
if ($totalSize < 1 || $totalSize > 2000 * 1024 * 1024) {
    errorResponse('Total video size is invalid.', 400);
}
if ($originalFilename === '' || !str_starts_with($mimeType, 'video/')) {
    errorResponse('Upload metadata is invalid.', 400);
}
...
try {
    $partPath = writeChunkFile($sessionId, $chunkIndex, $_FILES['file'], [
        ...
        'totalSize' => $totalSize,
        'filename' => normalizeUploadedName($originalFilename),
        'mimeType' => $mimeType,
    ]);
